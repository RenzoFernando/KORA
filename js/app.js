...
function showSettingsPanel() {
  const rows = DATA.settings
    .map(
      (item, index) =>
        `<article class="settings-row"><div><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.body)}</p></div><button class="switch-button ${settingEnabled(index) ? 'is-on' : ''}" type="button" data-toggle-setting="${index}"><span></span></button></article>`
    )
    .join('');

  showModal(
    `<h2>Ajustes de KORΛ</h2><p>Personaliza la experiencia de reproducción, privacidad y comunidad.</p><div class="settings-list">${rows}</div><button class="primary-button full" type="button" data-close-modal>Guardar ajustes</button>`
  );
}
...
function handleGlobalClick(event) {
  ...
  if (target.dataset.toggleSetting !== undefined)
    toggleSetting(Number(target.dataset.toggleSetting));
